...
export const TikTokVideoCard: React.FC<TikTokVideoCardProps> = ({
  ...
  useEffect(() => {
    ...
    const startSession = async () => {
      ...
      const res = await apiRequest('/watch-sessions/start', {
        ...
        body: JSON.stringify({
          ...
          deviceFingerprint: `web_canvas_${navigator.userAgent.replace(/[^a-zA-Z0-9]/g, '').slice(0, 16)}`,
        }),
      });

      if (isSubscribedMounted && res.success && res.data) {
        setSessionId(res.data.sessionId);
        setSessionToken(res.data.sessionToken);
        setRewardAmount(res.data.rewardAmount || initialReward);
      }
    };
    ...
    return () => {
      isSubscribedMounted = false;
      if (heartbeatIntervalRef.current) clearInterval(heartbeatIntervalRef.current);
    };
  }, [isActive, video.id, campaignId, initialMinWatch, initialReward, user]);

  // 2. Listen to YouTube postMessage events
  useEffect(() => {
    if (!isActive) return;
  ...
  // 3. Heartbeat synchronization — ONLY sends when video is actively playing
  useEffect(() => {
    if (!isActive || !sessionId || !sessionToken || rewardAwarded) return;
    ...
    heartbeatIntervalRef.current = setInterval(async () => {
      ...
      if (currentPositionRef.current >= initialMinWatch && !rewardAwarded) {
        const compRes = await apiRequest('/watch-sessions/complete', {
          method: 'POST',
          body: JSON.stringify({
            sessionToken,
            finalPositionSeconds: currentPositionRef.current,
          }),
        });

        if (compRes.success && compRes.data) {
          if (compRes.data.qualificationStatus === 'QUALIFIED' && compRes.data.rewardEarned > 0) {
            setRewardAwarded(true);
            setRewardToast(`🎉 You earned ₦${compRes.data.rewardEarned || rewardAmount}!`);
            refreshUser();
            if (heartbeatIntervalRef.current) clearInterval(heartbeatIntervalRef.current);
          }
        }
  ...
  // 1-Click Subscribe Link Handler (Deep Link with ?sub_confirmation=1)
  ...
  // Pure Video Embed URL with all controls, branding, and chrome hidden
  ...
  return (
    <div className="w-full h-full min-h-full max-h-full flex-shrink-0 relative flex items-center justify-center bg-black select-none overflow-hidden">
      ...
      <div className="w-full h-full min-h-full max-h-full md:max-w-[440px] md:h-[calc(100vh-2rem)] md:my-4 md:rounded-3xl relative bg-black overflow-hidden shadow-2xl border-0 md:border md:border-slate-800/80 flex items-center justify-center">
        ...
        {/* YOUTUBE IFRAME (Scaled 135% to completely hide all YouTube top & bottom UI chrome) */}
        {isActive ? (
        ...
        {/* TRANSPARENT TAP SHIELD (Tapping toggles mute / audio without touching YouTube internals) */}
        <div
        ...
        <div className="absolute top-0 left-0 right-0 p-4 pt-16 md:pt-4 bg-gradient-to-b from-black/85 via-black/30 to-transparent flex items-center justify-between z-20 pointer-events-none">
          {/* Sponsor Tag */}
          {hasReward ? (
            <div className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-500/20 border border-emerald-500/40 backdrop-blur-md">
        ...
        {isMuted && isActive && (
          <button
            ...
            className="absolute top-28 md:top-16 left-1/2 -translate-x-1/2 z-20 px-4 py-1.5 rounded-full bg-black/80 border border-emerald-500/40 text-emerald-300 text-xs font-bold shadow-lg backdrop-blur-md flex items-center gap-2 hover:scale-105 transition-all pointer-events-auto"
          >
            <VolumeX className="w-4 h-4 text-emerald-400" /> Tap for Sound
          </button>
        )}

        {/* RIGHT INTERACTION RAIL (TikTok Action Stack) */}
        <div className="absolute right-3 bottom-24 md:bottom-16 z-20 flex flex-col items-center gap-4 sm:gap-5 pointer-events-auto">
          ...
          <div className="relative flex flex-col items-center cursor-pointer group" onClick={handleSubscribe} title="1-Click Subscribe on YouTube">
            <div className="w-11 h-11 sm:w-12 sm:h-12 rounded-full border-2 border-emerald-400 p-0.5 bg-slate-900 shadow-xl overflow-hidden flex items-center justify-center font-bold text-white text-xs group-hover:scale-105 transition-transform">
              {channelThumbnail ? (
          ...
          <div className="flex flex-col items-center gap-1">
            <div
              className={`w-10 h-10 sm:w-11 sm:h-11 rounded-full backdrop-blur-md border flex items-center justify-center ${
                rewardAwarded
                  ? 'bg-emerald-500 border-emerald-400 text-slate-950 font-black shadow-lg shadow-emerald-500/50 animate-bounce'
                  : 'bg-black/50 border-emerald-500/40 text-emerald-400'
              }`}
            >
              {rewardAwarded ? <CheckCircle2 className="w-5 h-5 sm:w-6 sm:h-6 text-slate-950" /> : <DollarSign className="w-5 h-5" />}
            </div>
            <span className="text-[10px] font-bold text-emerald-300 drop-shadow-md">
              {rewardAwarded ? 'Earned' : `₦${rewardAmount}`}
            </span>
          </div>
        </div>

        {/* BOTTOM OVERLAY DETAILS (Correct Channel Display & Video Title) */}
        <div className="absolute bottom-0 left-0 right-0 p-4 pb-20 md:pb-6 bg-gradient-to-t from-black/90 via-black/70 to-transparent z-20 space-y-1.5 pointer-events-none">
          <div className="space-y-1 max-w-[78%] pointer-events-auto">
            ...
            <div className="flex items-center gap-2 cursor-pointer" onClick={handleSubscribe} title="View Channel">
              <span className="font-extrabold text-sm text-white drop-shadow-md tracking-tight hover:text-emerald-300 transition-colors">
                {channelHandle}
              </span>
        ...
        {rewardToast && (
          <div className="absolute top-1/3 left-1/2 -translate-x-1/2 z-30 px-5 py-3 rounded-2xl bg-emerald-500 text-slate-950 font-black text-sm shadow-2xl shadow-emerald-500/50 animate-bounce flex items-center gap-2 border-2 border-white pointer-events-auto">
        ...
        {copyToast && (
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 z-30 px-4 py-2 rounded-xl bg-slate-900/90 border border-sky-500/50 text-sky-300 font-bold text-xs shadow-2xl backdrop-blur-md flex items-center gap-2 pointer-events-auto">
            <Copy className="w-4 h-4 text-sky-400" /> Referral link copied to clipboard!
          </div>
        )}
      </div>
    </div>
